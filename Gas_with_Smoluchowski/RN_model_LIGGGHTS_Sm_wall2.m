...
N(end+1,1:4) = [0,0,Wall(3,1),inf];
N(end+1,1:4) = [0,0,Wall(3,2),inf];
...
C_List = contact_list(N(2:N_particles-1,:),cutoff_factor,direction_cond);
N_contactsA = size(C_List,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Bot = zeros(N_particles,1);
Top = zeros(N_particles,1);


for i = 2:1:N_particles-1
    if (N(i,direction_cond)-Wall(direction_cond,1) <= mean_r)%same temperature
        Bot(i,1) = N(i,8);
    end
end

for i = 2:1:N_particles-1
    if ((Wall(direction_cond,2)-N(i,direction_cond)) <= mean_r)%same temperature
             Top(i,1) = N(i,8);
    end
end
    ...
    Top = nonzeros(Top);
    Bot = nonzeros(Bot);
    C_List_count = size(C_List,1);
 for i=1:1:size(Top,1)
    j = C_List_count+i;
    P1 = Top(i,1);
    P2 = N(N_particles,8);
  ...
  C_List(j,3) = Wall(direction_cond,2) - N(P1,direction_cond);
 end
     C_List_count = size(C_List,1);
  for i=1:1:size(Bot,1)
    j = C_List_count+i;
    P1 = Bot(i,1);
    P2 = N(1,8);
  ...
  C_List(j,3) = Wall(direction_cond,1) - N(P1,direction_cond);
 end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:1:N_contacts
   ...
   if (Xi*r_eff < rmin)
    ...
    if h12<0
        ...
        theta1 = real(asind(r_c/r_1));
        theta2 = real(asind(r_c/r_2));
        C_List(i,5) = r_c;
...
%             C_c = pi*K_f*r_eff*log(alpha^2);
    ...
    if (C_s1 == 0)
    Cond(i) = 1/(1/C_c+1/C_s2);
    else 
        if (C_s2 == 0)
            Cond(i) = 1/(1/C_c+1/C_s1);
        else
...
Length_keff = Length_cell;%mean(N(Top,direction_cond))-mean(N(Bot,direction_cond)) ;
...
O_CN = 2*nnz(C_List(:,5))/(N_particles-2);
T_CN = 2*nnz(C_List(:,6))/(N_particles-2);
G_CN = 2*nnz(C_List(:,7))/(N_particles-2);

N_overlap = nnz(C_List(:,5));
N_touch = nnz(C_List(:,6));
N_gap = nnz(C_List(:,7));
