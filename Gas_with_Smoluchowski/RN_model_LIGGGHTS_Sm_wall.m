...
for i=1:1:size(Top,1)
  ...
  h12 = Wall(direction_cond,2) - N(P1,direction_cond) - N(P1,4);
  ...
  if h12<0
      r_c = sqrt(abs(h12)*r_eff/2);
      theta1 = asind(r_c/r_eff);
      Lo1 = r_eff*( 1- 0.5*( cos(theta1) + cos(Gamma_1) ) );
      Lo=Lo1+ h12;
        Hff = (visc/(Pressure*Lo))*sqrt(pi*8314*Tk/(2*mf));
        K_f = K_f0/(1+(2*bsbeta*Hff));
        C_List(j,12) = Lo;
...
%             C_c = pi*K_f*r_eff*log(alpha^2);
            ...
            N_gap = N_gap+1;
            C_List(j,7) = h12;
            C_List(j,8) = log(1 + (Xi^2)*r_eff/h12);
            C_c = pi*K_f*r_eff*log(1 + (Xi^2)*r_eff/h12);
        end        
%   (Wall(direction_cond,2)-Wall(direction_cond,1);

  end
    C_s1 = pi*K_s*(Xi*r_eff)^2/r_1;
    Cond(j) = 1/(1/C_s1+1/C_c);
    C_List(j,9) = Cond(j);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Con = sparse(C_List(:,1),C_List(:,2),Cond,N_particles+2,N_particles+2);
Con = Con + transpose(Con);

Cona = zeros(N_particles+2,1);

for i=1:1:N_particles+2
Cona(i) = sum(Con(i,:));
end
...
%%
Bot1 = zeros(N_particles,1);
Top1 = zeros(N_particles,1);
Vol = zeros(N_particles,1);
Inner_part = zeros(N_particles,1);

%%%Voltage initiation

    Vol(N_particles+1)= Top_temp;
    Vol(N_particles+2)=Bot_temp;   
    Inner_part(1:1:N_particles,1) = N(:,8);
        
Top1(1,1) = N_particles+1;
Bot1(1,1) = N_particles+2;        
...
I1 = zeros(N_particles+2,1);
I2 = zeros(N_particles+2,1);
