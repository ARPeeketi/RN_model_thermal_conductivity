...
%Air
% K_f0 =(-1*10^(-11))*(Tc.^3)  - (4*(10^(-8))*(Tc.^2)) + (8*(10^(-5))*Tc) + 0.0241;
% mf=28.96;
% dm = 3.66E-10;
...
requiredRk = 180E-6; %Scaling effects
...
Ann_File = zeros(Number_files,15);
counter_file =0;
...
for i=1:1:N_contacts
   ...
   if (Xi*r_eff < rmin)
    ...
    if h12<0
        ...
        theta_1 = acosd((d^2+r_1^2-r_2^2)/(2*d*r_1));
        theta_2 = acosd((d^2+r_2^2-r_1^2)/(2*d*r_2));
        r_c = real(r_1*sind(theta_1));
        C_List(i,5) = r_c;
        
        Lo1 = r_1*(Gamma_1*pi/180-sind(Gamma_1)-theta_1*pi/180+sind(theta_1))/(Gamma_1*pi/180-theta_1*pi/180);
        Lo2 = r_2*(Gamma_2*pi/180-sind(Gamma_2)-theta_2*pi/180+sind(theta_2))/(Gamma_2*pi/180-theta_2*pi/180);
        Lo=Lo1+Lo2;
        Hff = (kBo)*(Tk)/((sqrt(2))*(dm^2)*pi*Pressure*Lo);
        ...
        Lg1 = r_1*(Gamma_1*pi/180-sind(Gamma_1))/(Gamma_1*pi/180);
        Lg2 = r_2*(Gamma_2*pi/180-sind(Gamma_2))/(Gamma_2*pi/180);
        Lg = Lg1+Lg2+h12;
        C_List(i,13) = Lg;
        Hff = (kBo)*(Tk)/((sqrt(2))*(dm^2)*pi*Pressure*Lg);
        K_f = K_f0/(1+(2*bsbeta*Hff));
        alpha = K_s/K_f;
...
Ann_File(counter_file,1) = pforgk;
...
fprintf('K_sim \t K_ana \t Error \t Tco\n');
fprintf('%0.4f\t%0.4f\t%0.4f\t%0.4f\n\n',File_part(counter_file,13:15),Tco);
...
% A(:,1) = File_part(:,4).*File_part(:,18);
% A(:,2) = File_part(:,4).*File_part(:,18)+File_part(:,4).*File_part(:,19);
